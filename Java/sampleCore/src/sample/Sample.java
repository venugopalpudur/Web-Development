        ...
        do{                        
            ...
            switch(choise){
                case 1: 
                        System.out.println("Press\n1 - To Add Category & Weights \n2 - To Delete Category & Weights");
                         ...
                         if(dd==1){
                             ...
                             boolean sts=false;
                             for(String strr : DistCat){
                                 if(strr.equals(str)){
                                     sts=true;
                                 }
                             }
                             if(sts){
                                 System.out.println("Assginment Category already present, Hence cannot be added.");
                             }else{
                                 DistCat.add(str);
                                 wts.add(marks);
                             }
                             ...
                             boolean sts=false;
                             for(String strr : DistCat){
                                 if(strr.toLowerCase().equals(dell)){
                                     sts=true;
                                 }
                             }
                             if(sts){
                                 System.out.println(dell+" deleted successfully.");
                                 DistCat.remove(dell);
                                 int ind = wts.size()-1;
                                 wts.remove(ind);
                             }else{
                                 System.out.println("No data present for "+dell+" category !");
                             }                                                       
                         }else{
                             System.out.println("Wrong Choice");      
                         }
                         int[] wgt= new int[wts.size()];
                         String[] ct= new String[DistCat.size()];
                         for(int r=0; r<wts.size(); r++){
                             wgt[r]=wts.get(r);
                             ct[r]=DistCat.get(r);
                         }
                         assCatDist = Arrays.copyOf(ct, ct.length);
                         weights = Arrays.copyOf(wgt, wgt.length);
                ...
                case 2: System.out.print("Enter Serial No :");
                        ...
                        if(crud.addInfo(new info(sno, sname, subj, asscat, dos, pts))){
                ...
                case 4 : System.out.println("Enter the Student Serial Number to Update :");
                         int id = inp.nextInt();
                         if(crud.updateInfoById(id)){
                             System.out.println("Info Updated Successfully");
                         }else{
                             System.out.println("Update Unsuccessful");
                         }
                    break;
                case 5 : crud.showInfo();
                        System.out.println("");
                        System.out.printf("%-10s %-15s %n --------------------------------------------------------------------------------------%n", "S.No.","St Name");
                        for(int i=0; i<assCatDist.length; i++){
                            System.out.printf("%-10s %-15d %n", assCatDist[i], weights[i]);
                        }
                ...
                case 6 : System.out.println("Enter the Student Name to Calculate Rating :");
                         ...
                         if(!list.isEmpty()){
                         studentInfo stdi = new studentInfo(stname, sub, assCatDist, list, weights);
                         stdi.display();
                         }else{
                ...
                case 7 : System.out.println("Enter the Subject Name to Calculate Rating :");
                         ...
                         if(!list.isEmpty()){
                         subjectInfo subi = new subjectInfo(subname, StuName, assCatDist, list, weights);
                         subi.display();
                         }else{
                             System.out.println("Not Records Available. Insert the records to calculate.");
                         }
